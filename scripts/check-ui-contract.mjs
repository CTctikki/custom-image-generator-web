...
assert(styles.includes("clamp(300px, 21vw, 340px)"), "History sidebar should have enough dynamic width for timeline controls.");
...
assert(app.includes("studio-workspace") && styles.includes(".studio-workspace"), "Motion console and result workspace must share one shifting content column.");
assert(!app.includes("motion-console-stats"), "Motion console must not show task, frame, or quality stats.");
...
assert(app.includes("history-current-version") && styles.includes(".history-current-version"), "Open history sidebar must show the current version summary.");
assert(app.includes("history-timeline") && styles.includes(".history-timeline"), "History sidebar must render as a version timeline.");
assert(styles.includes(".output-panel::after"), "Result panel must draw a visual connection toward the open history timeline.");
...
assert(app.includes("result-showcase") && styles.includes(".result-showcase"), "Result panel must use a staged showcase layout.");
assert(app.includes("result-stage") && styles.includes(".result-stage"), "Result panel must render the main image stage.");
assert(app.includes("result-preview-strip") && styles.includes(".result-preview-strip"), "Result panel must render a thumbnail version strip.");
assert(app.includes("result-status-card") && styles.includes(".result-status-card"), "Result panel must render generation status below the stage.");
assert(app.includes("result-action-bar") && styles.includes(".result-action-bar"), "Result panel must expose bottom result actions.");
