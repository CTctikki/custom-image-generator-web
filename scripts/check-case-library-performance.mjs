...
assert(existsSync(new URL("../public/case-prompts/", import.meta.url)), "Expected per-case prompt files.");
...
const promptFiles = readdirSync(new URL("../public/case-prompts/", import.meta.url)).filter((name) => name.endsWith(".txt"));
...
assert(promptFiles.length === casesIndex.cases.length, "Prompt files should cover every case.");

assert(caseLibrary.includes('fetch("/cases-index.json")'), "Case library should load the lightweight index first.");
assert(caseLibrary.includes('fetch(`/case-prompts/${caseId}.txt`)'), "Case library should fetch prompt files by case id.");
assert(caseLibrary.includes("/case-prompts/"), "Case library should load prompts by case id.");
