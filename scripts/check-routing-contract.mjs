...
assert(api.includes('AA_IMAGE_MODEL_ID = "AA-gpt-image-2"'), "Browser model sorting must explicitly recognize the AA image model.");
assert(api.includes('return -2;') && api.includes("isAaImageModelId"), "Browser model sorting must put AA image model before SF and legacy gpt-image-2.");
assert(api.includes("BLOCKED_LEGACY_IMAGE_MODEL_IDS"), "Browser model list must hide legacy broken image2 aliases.");
assert(app.includes('PREFERRED_IMAGE_MODEL_ID = "AA-gpt-image-2"'), "Workspace model selection must prefer the AA image model when available.");
assert(app.includes('"ZN-gpt-image-2"'), "Workspace model migration must clear stored ZN defaults.");
