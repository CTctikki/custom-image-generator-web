...
线上版本部署为 Vercel 静态前端，`/api/*` 请求通过 `vercel.json` 转发到中转站：
...
本地开发仍可使用当前 Express 服务调试 `/api/models` 和 `/api/generate`。Vercel 部署时不会使用这个本地 Express 服务。
...
- 输出目录：`dist`
- `/api/:path*` → `https://api.lts4ai.com/api/:path*`
